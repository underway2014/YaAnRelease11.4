package Json
{
	 ...
	 * 
	 */
	public class WeatherXmlLoader extends EventDispatcher
	{
		...
		public function WeatherXmlLoader()
		{
			xmlNamesArr = [];
			xmlUrlArr = [];
			xmlClassArr = [];
		...
		public function getDataByName(name:String):Object
		{
			try
			{//   101271701
				var xmllist:XMLList = _data.city.(@cityname == name);
				return WeatherParse.parse(xmllist[0]);
			}
			catch(e:Error)
			{
			} 
		...
		public function getDataByUrl(_url:int):Object
		{
			try
			{//   101271701
				var xmllist:XMLList = _data.city.(@url == _url);
				return WeatherParse.parse(xmllist[0]);
			}
			catch(e:Error)
			{
